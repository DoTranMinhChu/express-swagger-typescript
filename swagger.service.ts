...
export class SwaggerService {
  ...
  public setSchemas(models: {
    ...
    for (const modelIndex in models) {
      ...
      for (const propertyIndex in model.properties) {
        ...
        if (property.format) newProperty.format = property.format;
        if (property.description)
          newProperty.description = property.description;
        if (property.enum) newProperty.enum = property.enum;
        if (property.example) newProperty.example = property.example;
  ...
  public addPath(args: IApiPathArgs, target: any): void {
    ...
    for (const controllerIndex in this.controllerMap) {
      ...
      if (controllerIndex === target.name) {
        ...
        if (args.description) currentController.description = args.description;
        if (args.security) currentController.security = args.security;
        if (args.deprecated) currentController.deprecated = args.deprecated;
      }
    }
  ...
  public addApiModelProperty(
    ...
    let swaggerBuildDefinitionModel: ISwaggerBuildDefinitionModel | undefined =
    ...
    if (args) {
      ...
      if (args.enum) swaggerBuildDefinitionModelProperty.enum = args.enum;
      if (args.itemType) {
      ...
      if (args.format) swaggerBuildDefinitionModelProperty.format = args.format;
      if (args.model) {
  ...
  private buildOperationRequestBody(
  ...
  private buildParameters(
    ...
    for (const parameterIndex in parameters) {
      ...
      if (parameter.type) newSwaggerOperationParameter.type = parameter.type;
      if (parameter.schema) {
        ...
        if (parameter.schema.model) {
          ...
          newSwaggerOperationParameter.schema.$ref = this.buildRef(
      ...
      if (parameter.items) newSwaggerOperationParameter.items = parameter.items;
      if (parameter.name) newSwaggerOperationParameter.name = parameter.name;
